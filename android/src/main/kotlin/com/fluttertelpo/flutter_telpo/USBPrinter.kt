...
class USBPrinter {
    ...
    //    private val picturePath: String = Environment.getExternalStorageDirectory().getAbsolutePath() + "/111.bmp"
...
//    fun CreateCode(str: String?, type: com.google.zxing.BarcodeFormat?, bmpWidth: Int, bmpHeight: Int): Bitmap {
...
//        val assetPath: String = mainActivity.binding.getFlutterAssets().getAssetFilePathBySubpath(imageUrl, "flutter_telpo")
        val inStream: InputStream = mainActivity.binding.getApplicationContext().getAssets().open(assetPath)
//        val inStream: InputStream = mainActivity.binding.getApplicationContext().getAssets().open("assets/" + assetPath)
        return getResizedBitmap(BitmapFactory.decodeStream(inStream), bmpWidth, bmpHeight);
    }
